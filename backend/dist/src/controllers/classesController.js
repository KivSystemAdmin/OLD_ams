...
exports.checkChildrenAvailabilityController = exports.checkDoubleBookingController = exports.createClassesForMonthController = exports.getInstructorClasses = exports.nonRebookableCancelController = exports.cancelClassController = exports.getClassesForCustomerCalendar = exports.getClassesForInstructorCalendar = exports.updateClassController = exports.getClassByIdController = exports.deleteClassController = exports.createClassController = exports.getClassesByCustomerIdController = exports.getAllClassesController = void 0;
...
const getAllClassesController = (_, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const classes = yield (0, classesService_1.getAllClasses)();
        const classesData = classes.map((eachClass) => {
            const { id, dateTime, customer, instructor, status, isRebookable, recurringClassId, } = eachClass;
            return {
                id,
                dateTime,
                customer: {
...
const getClassesByCustomerIdController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const id = Number(req.params.id);
    try {
        const classes = yield (0, classesService_1.getClassesByCustomerId)(id);
        const classesData = classes.map((eachClass) => {
            const { id, dateTime, customer, instructor, status, classAttendance, isRebookable, recurringClassId, } = eachClass;
            return {
                id,
                dateTime,
                customer: {
...
const createClassController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { classId, dateTime, instructorId, customerId, childrenIds, status, recurringClassId, } = req.body;
    ...
    if (!childrenIds)
        missingFields.push("childrenIds");
    if (!status)
        missingFields.push("status");
    if (!recurringClassId)
        missingFields.push("recurringClassId");
    ...
    try {
        ...
        const subscriptionId = yield prismaClient_1.prisma.$transaction((tx) => __awaiter(void 0, void 0, void 0, function* () {
            const recurringClass = yield (0, recurringClassesService_1.getRecurringClassByRecurringClassId)(tx, recurringClassId);
            if (!recurringClass) {
        ...
        const subscription = yield (0, subscriptionsService_1.getSubscriptionById)(subscriptionId);
        if (!subscription) {
            return res
                .status(400)
        ...
        const instructorBooked = yield (0, classesService_1.isInstructorBooked)(instructorId, dateTime);
        if (instructorBooked) {
            return res.status(400).json({
                error: "This instructor is already booked at the selected time. Please refresh your browser and try booking for a different time slot.",
            });
        }
        ...
        const [newClass, updatedClass] = yield Promise.all([
            ...
            (0, classesService_1.updateClass)(classId, undefined, // dateTime
...
// DELETE a class
const deleteClassController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
...
const updateClassController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { dateTime, instructorId, childrenIds, status, isRebookable } = req.body;
    try {
        const updatedClass = yield (0, classesService_1.updateClass)(classId, dateTime, instructorId, childrenIds, status, isRebookable);
        res.status(200).json({
...
const getClassesForInstructorCalendar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const instructorId = parseInt(req.params.instructorId);
    if (!instructorId) {
        return res.status(400).json({ error: "instructorId is required" });
    }
    try {
        const classes = yield (0, classesService_1.getClassesForCalendar)(instructorId, "instructor");
        const classesData = classes.map((eachClass) => {
...
const getClassesForCustomerCalendar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const customerId = parseInt(req.params.customerId);
    if (!customerId) {
        return res.status(400).json({ error: "customerIdId is required" });
    }
    try {
        const classes = yield (0, classesService_1.getClassesForCalendar)(customerId, "customer");
        const classesData = classes.map((eachClass) => {
            const { id, dateTime, instructor, status, classAttendance, isRebookable, } = eachClass;
            return {
                id,
                dateTime,
                instructor: {
...
// Cancel a class
const cancelClassController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const classId = parseInt(req.params.id);
    const isPastPrevDayDeadline = false;
    try {
        yield (0, classesService_1.cancelClassById)(classId, isPastPrevDayDeadline);
...
const nonRebookableCancelController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const classId = parseInt(req.params.id);
...
const getInstructorClasses = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const classes = yield (0, classesService_1.fetchInstructorClasses)(req.id);
        const classesData = classes.map((eachClass) => {
            const { id, dateTime, customer, instructor, status, classAttendance, isRebookable, } = eachClass;
            return {
                id,
                dateTime,
...
const createClassesForMonthController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const result = yield prismaClient_1.prisma.$transaction((tx) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const recurringClasses = yield (0, recurringClassesService_1.getValidRecurringClasses)(tx, firstDateOfMonth);
            // Get excluded classes.
            const recurringClassIds = recurringClasses.map((recurringClass) => recurringClass.id);
            const excludedClasses = yield (0, classesService_1.getExcludedClasses)(tx, recurringClassIds, firstDateOfMonth);
            ...
            const until = (0, dateUtils_1.getFirstDateInMonths)(firstDateOfMonth, 1);
            until.setUTCDate(until.getUTCDate() - 1);
            // Repeat the number of recurring classes.
            yield Promise.all(recurringClasses.map((recurringClass) => __awaiter(void 0, void 0, void 0, function* () {
                const { id, instructorId, startAt, endAt, subscriptionId, subscription, recurringClassAttendance, } = recurringClass;
                ...
                const firstDate = (0, dateUtils_1.calculateFirstDate)(firstDateOfMonth, dateUtils_1.days[startAt.getDay()], time);
                // Create the range of dates.
                const dateTimes = (0, dateUtils_1.createDatesBetween)(firstDate, endAt && endAt < until ? endAt : until);
                ...
                const isExistingClass = excludedClasses.some((excludedClass) => {
                    const excludedClassDateTimesStr = new Date(excludedClass.dateTime).toISOString();
                    const dateTimesStr = dateTimes.map((date) => new Date(date).toISOString());
                    return (dateTimesStr.includes(excludedClassDateTimesStr) &&
                        excludedClass.instructorId === instructorId);
                });
                if (isExistingClass) {
                    return;
                }
                const childrenIds = recurringClassAttendance.map((attendee) => attendee.childrenId);
                // Create the classes and its attendance based on the recurring id.
                yield (0, classesService_1.createClassesUsingRecurringClassId)(tx, id, instructorId, subscription.customerId, subscriptionId, childrenIds, dateTimes);
            })));
...
const checkDoubleBookingController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const isBooked = yield (0, classesService_1.checkDoubleBooking)(customerId, dateTime);
        if (isBooked) {
            return res.status(400).json({
...
const checkChildrenAvailabilityController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const childrenWithConflictingClasses = yield (0, classesService_1.checkForChildrenWithConflictingClasses)(new Date(dateTime), selectedChildrenIds);
...
exports.checkChildrenAvailabilityController = checkChildrenAvailabilityController;
